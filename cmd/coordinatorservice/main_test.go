...
func TestCoordinatorServiceCmd(t *testing.T) {
	...
	commonTests := []config.CommandTest{
		{
			...
			Args:              []string{"start", "--endpoint", "localhost:8004"},
			CmdLoggerOutputs:  []string{"Serving", "localhost:8004"},
			CmdStdOutput:      fmt.Sprintf("Starting %v", config.CoordinatorName),
			UseConfigTemplate: config.TemplateCoordinator,
			System:            s,
		},
		{
			Name:              "start",
		...
		},
		{
			...
			Args: []string{"version", "test"},
			Err:  fmt.Errorf(`unknown command "test" for "%v version"`, config.CoordinatorName),
		},
	}
	...
	for _, test := range commonTests {
		...
		t.Run(test.Name, func(t *testing.T) {
			config.UnitTestRunner(t, coordinatorserviceCmd(), tc)
		})
	}
}
