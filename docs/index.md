...
For production deployments, start with the [Deployment Guide](deployment-guide.md) for hardware sizing and topology, then the [Performance Tuning](performance-tuning.md) guide for configuration parameters. For a worked example, the evaluation notes in [`eval/`](../eval/) hold the complete setup that sustained 500,000 transactions per second on nineteen machines, what each change was worth, and a recreation of the Fabric-X paper's committer figures.
...
| [Performance](performance-tuning.md) | What each setting does; the [measured configuration](../eval/optimization-config.md) behind 500,000 tps, and [what each change was worth](../eval/optimization-summary.md) |
