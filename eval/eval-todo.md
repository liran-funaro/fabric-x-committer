...
| 2g | **Runs LAST, after batches 3b-9.** Measure the `insert_ns` rewrite: 5% at the 120-way split, plus the conflict-free hold as a regression check. Every earlier batch is its baseline and measures the path it removes. **`eval/workspace` already carries the rewrite, so a baseline bring-up must build with `d44ef3a4` reverted.** [ctx](#2g-the-insert_ns-rewrite) | new | blocked on 3b-9 |
