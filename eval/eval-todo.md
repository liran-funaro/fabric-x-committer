...
2. **[ ] Update the setup from what this experiment found**, and the eval documents with it
   (`cluster-optimization-log.md`, `optimization-issues.md`, `optimization-summary.md`).
3. **[ ] Validate every claim in the setup section**, and split setup from its justification so the
...
5. **[ ] Split the LaTeX into two parts**: committer-only setup + evaluation, then a new E2E section
   introducing the orderer setup, with the committer setup unchanged from part one.
6. **[ ] Plots full text width, larger fonts.** Currently too small to read.
