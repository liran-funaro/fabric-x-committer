...
| 2a-ii | **Re-run the tablet sweep at one fixed offered rate — the sweep so far is confounded.** `tab96` retires ~20,000 (level with 120, not the 25,100 a per-tablet law predicts), and the drain between its probes gives the reason: same tablets, same deployment, width 19% narrower, insert **1.325 s against 2.050 s**. Insert latency carries database queueing, so it is not a per-batch cost, and both it and the width move with how far above capacity a probe sat — 96's saturated width is 611 keys against 88's 301, twice as wide at *lower* throughput. So no per-batch cost law stands, including per-tablet. Design: **15,000 tps at every tablet count** (under 20,000 at 96/120 and 27,400 at 88), 300 s, width and insert read in-window. Backlog near zero everywhere, tablet count the only difference. | new | **do before any more tablet points** |
