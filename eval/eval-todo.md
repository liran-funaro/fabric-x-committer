...
| 2a | **Tablet sweep at 5% conflicts, plotted against `tablets × keys` rather than tablets.** The width that reaches `insert_ns` is not the graph's 500-transaction chunk — the VC re-batches, and the measured median is 152 transactions (~304 keys, n=14,682), which puts the crossing near **108** tablets, not 32. So 8/16/32/64 all sit under the threshold at that width and a sweep on tablets alone can show nothing while looking like a refutation. Worse, the width falls from ~300 to ~125 as the collapse sets in, so it moves with what is being measured. Record keys-per-insert at each point (`tx_per_db_batch` is already derived) and the sweep interprets itself either way.<br><br>**Two width measurements disagree and the crossing moves with them.** The median 152 tx (~304 keys, crossing ~108 tablets) is taken across the run; two 40-second windows on one VC's own counters, at table sizes 61% apart, both give **172–175 tx (~349 keys, crossing ~93)** and agree with each other to 2%. The likely reconciliation is that the median spans the ramp into collapse, where width genuinely falls, while the steady state is 172–175 — which is also why 2a saw ~300 falling to ~125 and a steady-state sample does not. Either way the instruction stands and matters more than the number: **sample width in-window at each tablet count**, because width follows the downstream service rate, so a tablet change that moves throughput moves width and therefore the predicted edge. Assuming a fixed 349 or 304 can make a holding model look falsified. | new | **running (`tab96`)** |
| 2a-i | **What the sweep has priced so far, and what `tab96` is predicted to do.** 120 tablets retires 20,389 tps (flat within 2.1%) at a 1.690 s insert and a 349-key width; 88 tablets retires **25,000–27,700** at 1.170 s and 295 keys. Both sit under the 32,768 threshold at their measured width (88 x 295 = 25,960), so the *threshold* model is refuted for this path — cost is continuous in keys x tablets, 45.1 µs per key-tablet at 88 and 40.4 at 120. **Two models now predict `tab96` differently, and it is mid-flight, so this is a real pre-registration.**<br>*(a) per key x tablet*, from 40.8 µs at 120: insert **1.14–1.28 s**. *(b) per tablet alone*, ~13.9 ms each: insert **1.34 s**.<br>The width is what separates them, and the width moved: `tx_per_insert` at 88 tablets reads 84.2–84.5 transactions, so **168 keys**, not the 295 assumed earlier. Re-fitting on that, per-key-tablet is **98% apart** between the two points (80.9 µs at 88 against 40.8 at 120) while per-tablet is **4% apart** (13.8 ms against 14.2). So (b) already fits and (a) does not — the failure path's `key = ANY(_keys)` appears unable to prune tablets and fans out to all of them at a fixed cost each, independent of how many keys it is looking for. That also kills the 32,768 threshold outright: 168 x 88 = 14,784, less than half the threshold, and the insert is still 1.2 s. **Result: 1.317 s, so (b) and not (a).** Predicted 1.34 against 1.317 measured, 1.7% apart, while (a)'s band was 1.14–1.28 and excluded it. Throughput came in at 28,537 tps, just over (a)'s 25,300–28,300 band, so the insert is what discriminated — as it had to, since throughput at these rates is a queue depth as much as a cost. The line is now 1.212 s at 88, 1.317 at 96 and 1.709 at 120, i.e. **13.8 ± 0.4 ms per tablet, flat across an 8x range of offered rate and a doubling of width** (84 to 175 transactions per insert). The 96 → 120 step is what excludes everything else: width doubled *and* tablets rose 25%, so per-key-x-tablet predicts 3.35 s and per-key 2.68 s against 1.709 measured. **`insert_ns`'s failure path cannot prune tablets — it fans out to all of them at a fixed cost each, independent of how many keys it is looking for.** | `9c-ds5-tab96` | **done, model settled** |
| 2a-ii | **The bound, and the first conflicting point that can meet it.** At ~1.8 attempts per commit the 1 s bound sits at `tablets x 13.8 ms x 1.8 < 1000`, i.e. **~40 tablets**. So 64 (0.88 s) and 160 (2.21 s) are predicted misses on a slope three points already fix, and they were dropped for the pair that brackets the bound by eight either side: **32 tablets → 0.79 s per commit, should MEET; 48 → 1.19 s, should MISS.** That pair produces the first conflicting operating point at a tablet count anyone would run. 8 tablets predicts a 0.11 s insert, which is why it reached 172,000 tps, and confirming that anchors the low end of the line. | `9c-ds5-split32`, `9c-ds5-tab48`, `9c-ds5-ladder8tab`, `9c-ds5-hold8` | queued |
| 2b | **Conflict-share sweep**: 1%, 0.1%, **0.01% and 0.001%**. The last two were added because the first two cannot bracket anything. p99 is taken over *valid* transactions and a failing batch drags its valid ones through the retry with it, so the share of **batches** holding a conflict decides the tail: at the measured width of ~175 transactions that is `1-(1-p)^175` = 83% at 1%, 16% at 0.1%, 1.7% at 0.01%, 0.18% at 0.001%. A 99th percentile absorbs 1% of transactions and no more, so **the crossing is at ~0.006%**, between the last two — and 1% and 0.1% are both predicted misses that would locate nothing. With all four the sweep returns the share at which this tablet layout becomes usable, which is the number an operator needs. Every point also prices its share under saturation whether or not it meets the bound. | `9c-ds1`, `9c-ds01`, `9c-ds001`, `9c-ds0001` | queued |
