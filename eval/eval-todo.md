...
| 2a-i | **What the sweep has priced, and the width figure that was wrong.** 120 tablets retires 20,389 tps at a 1.690 s insert; 88 tablets retires **25,000–27,700** at 1.194 s; `tab96` is in flight and retiring ~22,000, so cost is monotone in tablets. The width that fed the model was wrong by the retry count: `committed/insert_count` = 84 tx is the width *divided by attempts*, because `insertStates` re-sends the whole batch on every attempt and the retry drops only the transactions the last one invalidated. Real width is **150.5 tx = 301 keys** at 88 (316 on the first attempt), against 350 at 120 — 16% apart, not halved.<br><br>**So the two tablet points barely discriminate.** Predicting 120 from 88: per-tablet −4.2%, per-key×tablet +11.4%, per-key −18.3%. Per-tablet wins and per-key is dead, but by 4% against 11%, not the 4% against 98% that the divided width produced. **The test that does discriminate is already in hand**: at *fixed* 88 tablets the width moves 287→535 keys across 384 intervals and the insert moves only 1.14→1.30 s (R²=0.038 on width). One run, one deployment, no cross-day drift — that is the evidence that the cost is per-tablet. It is not per-tablet exactly: ms/tablet still climbs 14% across that sweep.<br><br>**The 32,768 threshold is refuted for this path**: 301 × 88 = 26,488 is under it and the insert is still 1.2 s, and the 88→120 step is 42%, not a factor of 24. §6's constant stands for the read path it was fitted to. **And the failure path costs ~2.6 s, not 1.2 s** — the conflict histogram is in the code but not on the deployed binary, so the single histogram blends 1 clean attempt with 0.79 failing ones. Ratios between tablet counts survive; absolute per-attempt costs should not be quoted until it is deployed. | `9c-ds5-tab96` | corrected, `tab96` running |
