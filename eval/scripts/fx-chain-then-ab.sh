...
# Derive the expected count from the chain rather than hard-coding it. It was hard-coded to 9 and the
# chain has 11 batches, so two could have failed and the A/B would still have started -- and the A/B is
# the one step that cannot be taken back.
WANT=$(grep -c "^run \|^runskip " fx-plan-14x-lf.sh 2>/dev/null || echo 0)
DONE=$(grep -c " done$" plan-14x-lf.log 2>/dev/null || echo 0)
FAILED=$(grep -c "FAILED rc=" plan-14x-lf.log 2>/dev/null || echo 0)
echo "### $(date +%H:%M:%S) 14x finished: $DONE of $WANT batches done, $FAILED failed"
if [ "$WANT" = "0" ] || [ "$DONE" -lt "$WANT" ]; then
  echo "### $(date +%H:%M:%S) only $DONE of $WANT batches completed -- NOT starting the A/B."
