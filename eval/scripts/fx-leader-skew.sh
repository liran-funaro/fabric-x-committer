...
# at 17.0 ms and 262 ms. Ruled out already -- an undrained pipeline (the table was wiped, SST 33.4 GB ->
...
# tablet, so if a fresh table puts several leaders on one tablet server, every batch waits on that server
# and the per-batch cost rises without the layout looking any different.
#
...
try:
    ...
    sys.exit()
# The `user` key, not `tables` -- the top level is {user, index, system} and a lookup for "tables"
# silently yields nothing, which reads as "no tables exist" rather than as a wrong key.
for t in d.get("user", []):
    ...
    print("%s %s %d %d %d %s" % (time.strftime("%H:%M:%S"), n, running,
                                 len(leaders), max(leaders.values()) if leaders else 0, dist or "-"),
          flush=True)
PY
  sleep 60
done
