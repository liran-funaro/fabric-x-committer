...
# Immediately after, on that same deployment: does the advantage survive the table ageing? The layout rests
# at 12 running tablets because that is 1.0 per tablet server, and the next split needs each tablet to cross
# the HIGH phase threshold of 10 GiB -- so ~120 GiB of table before the count can move at all, against the
# 13.5 GB measured during the ds5 ladder. That bounds the risk arithmetically rather than leaving it open,
# and this rung checks the arithmetic against a table that has just carried three rungs at 150k-350k.
