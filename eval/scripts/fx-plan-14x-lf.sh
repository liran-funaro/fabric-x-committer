...
# FIRST: re-run the 20% and 30% shares on the unified rate list. They ran at 10k/25k/50k/80k while 5% and
# 10% ran at 15k/30k/60k/100k, and `throughput()` counts transactions FINISHED -- the paper's convention,
# since its Figure 9b shows throughput rising with the invalid share. So the panel series would plot each
# share's top rung and slope down above 10% purely because of the ladder, while every share in fact met
# every rung it was offered at ~190 ms and 10% CPU. That is an artefact presented as a finding, in the
# section's headline figure. The two lists share no rate at all, so no common-rate comparison rescues it
# either. REDO is required because both already have met rows and would otherwise be skipped.
# The two share re-runs are DONE and are not repeated: nosplit20 finished at 14:42 on the unified rate
# list (its 100,000 rung met at 193 ms) and nosplit30 is finishing now. They carry REDO, so leaving them in
# would force ~50 minutes of re-running two batches that just completed -- REDO overrides the
# already-measured check that would otherwise skip them.
...
run nosplithi 9c-nosplit-ds5-hi $NS 20000
...
run hold8nosplit 9c-ds5-hold8-nosplitting $NS
...
run tabhold 9c-ds5-tabhold $NS 2000
