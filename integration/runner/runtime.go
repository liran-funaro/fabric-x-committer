...
type (
	...
	CommitterRuntime struct {
		...
		OrdererStream       adapters.Broadcaster
...
func NewRuntime(t *testing.T, conf *Config) *CommitterRuntime {
	...
	// NewOrdererTestEnv is used here to allocate orderer servers to parties and create
	// the appropriate config block and artifacts with the given servers. While this test
	// environment is primarily designed for controlling an ordering server for test, we reuse it
	// here to avoid duplicating the server allocation and artifact generation logic.
	// It is also used as an ordering server controller in the config update tests.
...
func (c *CommitterRuntime) OpenNotificationStream(ctx context.Context, t *testing.T) {
	...
	require.NoError(t, err)
	c.StreamAllTxStream, err = c.NotifyClient.StreamAllTransactions(
		ctx,
		&committerpb.StreamAllRequest{},
	)
	require.NoError(t, err)
}
