...
func TestBroadcast(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{tlsMode: connection.NoneTLSMode, ftLevel: ordererdial.BFT},
		{tlsMode: connection.OneSideTLSMode, ftLevel: ordererdial.BFT},
		{tlsMode: connection.MutualTLSMode, ftLevel: ordererdial.BFT},
		{tlsMode: connection.MutualTLSMode, ftLevel: ordererdial.CFT},
	} {
		t.Run(fmt.Sprintf("tls-mode:%s %s", tc.tlsMode, tc.ftLevel), func(t *testing.T) {
			t.Parallel()
			e := newBroadcastTestEnv(t, tc.tlsMode, tc.ftLevel)
			expectedSuccess := 1
			if tc.ftLevel == ordererdial.BFT {
				expectedSuccess = 3
			}
...
func TestBroadcastBFT(t *testing.T) {
	...
	e := newBroadcastTestEnv(t, connection.MutualTLSMode, ordererdial.BFT)
	t.Log("All good")
...
func (e *broadcastTestEnv) submit(
	...
	defer connection.CloseConnectionsLog(stream)
