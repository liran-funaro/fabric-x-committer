...
func newBroadcastStream(
	ctx context.Context, connections []*grpc.ClientConn,
) (*broadcastStream, error) {
	streams := make([]ab.AtomicBroadcast_BroadcastClient, len(connections))
	errs := make([]error, len(connections))
	for i, c := range connections {
		streams[i], errs[i] = ab.NewAtomicBroadcastClient(c).Broadcast(ctx)
	}
