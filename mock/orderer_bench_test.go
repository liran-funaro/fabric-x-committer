...
func BenchmarkOrdererBlockRate(b *testing.B) {
	...
	// measurement, and it is identical for all of them.
	env := NewOrdererTestEnv(b, &OrdererTestParameters{OrdererConfig: &OrdererConfig{
	...
	for _, txCount := range []int{500, 10_000} {
		for _, inPlace := range []bool{false, true} {
			b.Run(fmt.Sprintf("prepare-in-place=%v/tx=%d", inPlace, txCount), func(b *testing.B) {
				benchmarkBlockRate(b, env.ArtifactsPath, txCount, inPlace)
			})
		}
	}
}

func benchmarkBlockRate(b *testing.B, artifactsPath string, txCount int, inPlace bool) {
	b.Helper()
	orderer, err := NewMockOrderer(&OrdererConfig{
		ArtifactsPath:    artifactsPath,
		SendGenesisBlock: false,
		BlockSize:        1,
		// Two blocks, deliberately: SubmitBlock only hands the block to the preparing goroutine, so a
		// deep buffer would measure the channel write and never the preparation behind it. At a depth
		// of two the submitter can run at most one block ahead and is paced by the stage under test.
		OutBlockCapacity: 2,
	...
	// A ring of distinct blocks, not one block submitted repeatedly. In-place preparation writes the
	// header into the block it was given, so a submitter that reuses one block hands the orderer's cache
	// several entries that are all the same object, and a consumer asking for block n can wait for a
	// number that has already been overwritten. That is the contract PrepareInPlace documents.
	blocks := make([]*common.Block, 8)
	for i := range blocks {
		blocks[i] = benchBlock(txCount, inPlace)
	}
	i := 0
	b.ResetTimer()
	for b.Loop() {
