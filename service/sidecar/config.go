...
type (
	...
	Config struct {
		...
		Orderer   ordererdial.Config       `mapstructure:"orderer" default:"suspicion-grace-period-per-block=1s"`
	...
	LedgerConfig struct {
		Path string `mapstructure:"path" default:"./ledger/"`
	...
	NotificationServiceConfig struct {
		...
		MaxTimeout time.Duration `mapstructure:"max-timeout" default:"1m" validate:"gt=0"`
		// MaxActiveTxIDs is the global limit on total active txID subscriptions across all streams.
		MaxActiveTxIDs int `mapstructure:"max-active-tx-ids" default:"100000" validate:"gt=0"`
		// MaxTxIDsPerRequest is the maximum number of txIDs allowed in a single notification request.
		MaxTxIDsPerRequest int `mapstructure:"max-tx-ids-per-request" default:"1000" validate:"gt=0"`
		...
		StreamWriteTimeout time.Duration `mapstructure:"stream-write-timeout" default:"30s" validate:"gt=0"`
	}
)
