...
func TestQueueSizeMetricsAreWired(t *testing.T) {
	...
	q := &queues{
		...
		txsStatus:       make(chan *committerpb.TxStatusBatch, 4),
	}
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			gauge: func() int { return test.GetIntMetricValue(t, m.batcherInputQueueSize) },
		},
		{
			name:  "preparer input",
			fill:  func() { q.toPrepareTxs <- &servicepb.VcBatch{} },
			gauge: func() int { return test.GetIntMetricValue(t, m.preparerInputQueueSize) },
		},
		{
			name:  "validator input",
			fill:  func() { q.preparedTxs <- &preparedTransactions{} },
			gauge: func() int { return test.GetIntMetricValue(t, m.validatorInputQueueSize) },
		},
		{
			name:  "committer input",
			fill:  func() { q.validatedTxs <- &validatedTransactions{} },
			gauge: func() int { return test.GetIntMetricValue(t, m.committerInputQueueSize) },
		},
		{
			name:  "tx status output",
			fill:  func() { q.txsStatus <- &committerpb.TxStatusBatch{} },
			gauge: func() int { return test.GetIntMetricValue(t, m.txStatusOutputQueueSize) },
		},
	} {
