...
func TestNewIdentitySigner(t *testing.T) {
	...
	for _, tc := range []struct {
		name   string
		config *ordererdial.IdentityConfig
	}{
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			signer, err := ordererdial.NewIdentitySigner(tc.config)
...
func mspDirToIdentityConfig(d *msp.DirLoadParameters) *ordererdial.IdentityConfig {
