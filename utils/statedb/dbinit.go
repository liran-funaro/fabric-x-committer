...
func NewPool(ctx context.Context, config *Config) (*pgxpool.Pool, error) {
	...
	return retry.ExecuteWithResult(ctx, config.Retry, func() (*pgxpool.Pool, error) {
		...
		// The caller only closes a pool it receives, so an unusable one is closed here
		// rather than leaked once per attempt.
		p.Close()
		var pgErr *pgconn.PgError
		if errors.As(pingErr, &pgErr) && pgErr.Code == pgerrcode.InvalidCatalogName {
			return nil, errors.Wrapf(errors.Join(retry.ErrNonRetryable, ErrDatabaseNotFound),
				"database %s", config.Database)
		}
		return nil, errors.Wrap(pingErr, "failed to create a connection pool")
